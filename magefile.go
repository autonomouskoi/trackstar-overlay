...
func Release() error {
	...
	filename := fmt.Sprintf("trackstar-overlay-%s.akplugin", version)
...
func WASM() error {
	...
	outFile := filepath.Join(pluginDir, "overlay.wasm")
...
func Manifest() error {
	...
	manifestPB := &modules.Manifest{
		...
		WebPaths: []*modules.ManifestWebPath{
			...
			{
				Path:        "/m/trackstar-overlay/ui.html",
